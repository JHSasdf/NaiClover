...
|한우리|풀스택|팔로우 기능 백엔드, 알림 기능,DB틀 구축, correcting page 프론트 백, ErrorLog페이지 프론트, 백엔드|
|윤정훈|풀스택|채팅 로직 구현, 모노챗 언어 제한 기능, 팔로잉, 팔로워 인원 로직 구현|
|선지훈|풀스택|회원가입 풀스택, 로그인 풀스택, 마이페이지 백엔드 구축, searchUser페이지 백엔드, post페이지 백엔드 구축, 개인챗, 모노챗 백엔드, 현재 채팅 참여하고 있는 인원 로직, 채팅 읽은 인원 업데이트 로직 구현, 프론트 백 오류 수정, 404, 500 등 오류페이지 프론트, 백엔드 구축, 서버구축, 배포|
