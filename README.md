...
|선지훈|풀스택|회원가입 풀스택, 로그인 풀스택, 마이페이지 백엔드 구축, searchUser페이지 백엔드, post페이지 백엔드 구축, 개인챗, 모노챗 백엔드, 현재 채팅 참여하고 있는 인원 로직, 채팅 읽은 인원 업데이트 로직 구현, 프론트 백 bug fix, 404, 500 등 오류페이지 프론트 페이지 백엔드 구축, 서버구축, 배포|
